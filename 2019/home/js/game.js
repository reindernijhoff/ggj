...
function init() {
	...
	const triangle = Bodies.polygon(400, 200, 3, PLAYER_RADIUS / 1.2, {frictionAir: 0.0, density: 0.002});
	const cube = Bodies.polygon(800, 200, 4, PLAYER_RADIUS, {frictionAir: 0.0});
	...
	Events.on(engine, "beforeUpdate", () => {
		...
		if (GAME_STATE == 2) {
			...
			if (timeLeft < GAME_STATE_3_DURATION) {
				...
				updateBody(triangle, 65, 68, 83, /*Math.PI / 3*/0, cubeParticles, engine);
				updateBody(cube, 74, 76, 75, 0, triangleParticles, engine);
			} else {
				GAME_STATE = 0;
...
function updateBody(body, l, r, f, rOffset, particles, engine) {
	body.torque = (keyCodes[l] ? -torque : 0) + (keyCodes[r] ? torque : 0);
	if (keyCodes[f]) {
		const s = .001;
